...
Pod RPC failures also carry their method name and observed HTTP status when available. That status can be 200 when the failure follows successful headers; the reason still identifies the failure. Wake failures label that failure with `failureStage` (`precheck`, `submission`, or `readiness`); the reason and detail fields always describe the same observation. Later readiness success does not make an uncertain wake submission certain, and may retain its submission failure details.

Non-overview status and wake terminal results include integer `serverElapsedMs`, measured with a monotonic clock from handler entry, before admission, to result fitting. It includes successful admission and server-side Cursor round trips, excludes MCP transport/client time, and is neither a deadline nor a phase timing. Admission exceptions retain their existing shared refusal/error response without this field.
