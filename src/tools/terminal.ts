...
export function registerTerminalTools(server: McpServer, policy: Policy, apiKey = '',
  ...
  add('execute', 'Run Bash once in /tmp. Use status sessionId and a unique commandId; poll read. Identical requests deduplicate. Never retry uncertain execution under a new ID. Combined PTY output; startup profiles disabled.',
  ...
  add('session_input', 'Send UTF-8 text/control keys using nextInputSequence; newline submits, Ctrl-C is U+0003, EOF U+0004. Latest identical input deduplicates. inputOutcome is delivery, not execution. Never resend uncertain input under a new sequence.',
